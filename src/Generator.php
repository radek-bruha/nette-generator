...
class Generator {
	...
	public function run() {
		...
		$this->fixMissingKdybyConsole();
		$netteContainer = $this->getNetteContainer();
		if(php_sapi_name() !== 'cli') {
			define('STDIN', fopen('php://stdin', 'r'));
			echo '<pre>';
		}
		CLI::write('Welcome to the Nette Framework CRUD generator 1.0@beta6.', 0, FALSE);
	...
	private function buildFromEntities($usedEntities = []) {
	...
	/** Fix missing Kdyby\Console extension, which prevents Nette container to be built */
	private function fixMissingKdybyConsole() {
		$config = file_get_contents(static::$netteDirectory . '/config/config.neon');
		if(mb_strrpos($config, 'extensions:') === FALSE) $config .= PHP_EOL . 'extensions:' . PHP_EOL . "\t" . 'console: Kdyby\Console\DI\ConsoleExtension';
		if(mb_strrpos($config, 'Kdyby\Annotations\DI\AnnotationsExtension') === FALSE) {
			$config = str_replace('extensions:', 'extensions:' . PHP_EOL . "\t" . 'annotations: Kdyby\Annotations\DI\AnnotationsExtension', $config);
		}
		if(mb_strrpos($config, 'Kdyby\Events\DI\EventsExtension') === FALSE) {
			$config = str_replace('extensions:', 'extensions:' . PHP_EOL . "\t" . 'events: Kdyby\Events\DI\EventsExtension', $config);
		}
		if(mb_strrpos($config, 'Kdyby\Console\DI\ConsoleExtension') === FALSE) {
			$config = str_replace('extensions:', 'extensions:' . PHP_EOL . "\t" . 'console: Kdyby\Console\DI\ConsoleExtension', $config);
		}
		if(mb_strrpos($config, 'Kdyby\Replicator\DI\ReplicatorExtension') === FALSE) {
			$config = str_replace('extensions:', 'extensions:' . PHP_EOL . "\t" . 'replicator: Kdyby\Replicator\DI\ReplicatorExtension', $config);
		}
		if(mb_strrpos($config, 'Kdyby\Translation\DI\TranslationExtension') === FALSE) {
			$config = str_replace('extensions:', 'extensions:' . PHP_EOL . "\t" . 'translation: Kdyby\Translation\DI\TranslationExtension', $config);
		}
		file_put_contents(static::$netteDirectory . '/config/config.neon', $config);
	}

	/** Check Nette extensions configuration */
	private function checkExtensionsConfiguration() {
		$newInstall = FALSE;
	...
	private function processSourceDialog($netteContainer) {
		...
		try {
			$database = static::$settings->netteDatabase;
			$database = new \PDO("mysql:dbname=$database->database;host=$database->hostname", $database->username, $database->password, [\PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES utf8mb4']);
	...
	private function repairDatabaseConfiguration($database) {
		...
		$this->checkDatabaseConfiguration($database, function($code, $message, $database) {
			...
			switch((int)$code) {
				case 2002:
					CLI::write("Database server '$database->hostname' not found.", 2);
					break;
				case 1044:
					CLI::write("Access denied for user '$database->username' to database '$database->database'.", 2);
					break;
				case 1045:
					CLI::write("Access denied for user '$database->username'@'$database->password'.", 2);
					break;
				case 1049:
					CLI::write("Unknown database '$database->database'.", 2);
					break;
				default:
					CLI::write($message);
					break;
		...
		$config['database']['dsn'] = "mysql:host=$database->hostname;dbname=$database->database";
		$config['database']['user'] = $database->username;
		$config['database']['password'] = $database->password;
		$config['database']['reflection'] = 'discovered';
